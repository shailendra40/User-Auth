...
// Signup route
app.post('/signup', uploadProfileImg.array('image', 5),uploadDocs.array('documents', 5), async (req, res) => {
    try {
...
if (!passwordRegex.test(password)) {
    return res.status(400).json({ error: 'Password must be 8 characters with upper, lower, digit, and special char' });
        ...
        const invalidImages = images.filter(image => !allowedImageExtensions.includes(path.extname(image.originalname)));
        ...
        const allowedDocExtensions = ['.pdf', '.xlsx', '.docx', '.txt'];
        const invalidDocuments = documents.filter(doc => !allowedDocExtensions.includes(path.extname(doc.originalname)));
