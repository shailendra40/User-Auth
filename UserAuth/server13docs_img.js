...
// const authenticateToken = require('./middleware/authenticateToken'); // Assuming you save the middleware as authenticateToken.js
...
function authMiddleware(req, res, next) {
  ...
  if (token == null) {
    return res.status(401).json({ error: 'No token provided' }); // Unauthorized if no token provided
  }

  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, user) => {
    if (err) {
      return res.status(403).json({ error: 'Token is invalid' }); // Forbidden if token is invalid
    }
...
imageRouter.post("/verify-otp", async (req, res) => {
  try {
    ...
    // Send Verification Success Email function
    async function sendVerificationSuccessEmail(userEmail) {
      try {
        // Create a Nodemailer transporter
        const transporter = nodemailer.createTransport({
          service: "gmail",
...
You can now log in and enjoy all the features our platform has to offer.
...
// Retrieve all image
imageRouter.get("/users", authMiddleware, async (req, res) => {
  try {
...
// Profile route
imageRouter.get("/profile", authMiddleware, async (req, res) => {
  try {
...
imageRouter.patch("/profile/update", authMiddleware, async (req, res) => {
  const { name,username } = req.body;
  const user = await prisma.userAuthDetails.update({
    where: {
      id: req.user.id,
    },
    data: {
...
//   try {
//     const userId = req.user.id;
//     const { username,name } = req.body; // Only allowing username to be updated

//     // Fetch current user details to preserve the email
//     const currentUser = await prisma.userAuthDetails.findUnique({
//       where: { id: userId },
//     });
...
//       where: { id: userId },
//       data: {
//         username: username || currentUser.username, // Update username if provided, otherwise keep the existing one
//         name: name || currentUser.name // Update name if provided, otherwise keep the existing one
//       },
//       select: {
//         id: true,
//         username: true,
//         email: true, // Returning email to show it remains unchanged
//         name: true, // Add any other fields you want to include in the response
//       },
//     });
